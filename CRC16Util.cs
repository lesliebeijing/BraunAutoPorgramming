...
namespace BraunAutoProgramming
{
    class CRC16Util
    {
        public static int CRC16BEGIN = 65535;
        private static int[] CRC16_Lookup = {0, 4129, 8258, 12387, 16516,
        ...
        public static int calculate(int crc16, string s)
        {
            try
            {
                byte[] bytes = Encoding.UTF8.GetBytes(s);
                int i = 0;
                for (int n = bytes.Length; i < n; i++)
                {
                    char c = (char)bytes[i];
                    int intValue = (int)c & 0xFF;
                    crc16 = intValue ^ (crc16 & 0xFF) << 8 ^ CRC16_Lookup[(crc16 >> 8)];
                }
            }
            catch (Exception)
            {
